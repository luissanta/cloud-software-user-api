...
def signup():
    try:
        ...
        if not passValid.match(user.password1):
            return {
                "error": "invalid password, check: size between 6-20 characters, have at least one Uppercase and Lowercase letter, at least one number and at least one special character"}, 400
        ...
        newUser = User(username=user.username, password=user.password1, email=user.email)
